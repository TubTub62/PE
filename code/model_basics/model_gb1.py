...
clear()
x_train, x_test, y_train, y_test = preprocess("data/gb1/", "gb1", num_processes=12)

def model_gb1(x_train, y_train, x_test, y_test, epochs=20, batch_size=64):
    model = Sequential()
    model.add(keras.Input((25,56)))
    model.add(Conv1D(filters=256, kernel_size=8, activation='relu', use_bias=True, padding='same'))
    model.add(Conv1D(filters=128, kernel_size=8, activation='relu', use_bias=True, padding='same'))
